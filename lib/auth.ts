...
export function findUserByIdentifier(id: string): UserRow | undefined {
  // If it looks like an email, match by email; otherwise treat as a
  // username. Both lookups are case-insensitive.
  if (id.includes('@')) {
    return one<UserRow>(`SELECT * FROM users WHERE LOWER(email) = LOWER(?)`, [id]);
  }
  return one<UserRow>(`SELECT * FROM users WHERE LOWER(username) = LOWER(?)`, [id]);
}
...
export function createUser(args: {
  email: string; password: string; name: string; role: Role; username?: string | null;
}): User {
  if (findUserByEmail(args.email)) throw new Error('email already registered');
  if (args.username) {
    const existing = one(`SELECT id FROM users WHERE LOWER(username) = LOWER(?)`, [args.username]);
    if (existing) throw new Error('username already taken');
  }
  const hash = hashPassword(args.password);
  const info = run(
    `INSERT INTO users (email, username, password_hash, name, role) VALUES (?, ?, ?, ?, ?)`,
    [args.email.toLowerCase(), args.username ?? null, hash, args.name, args.role],
  );
  return one<User>(
    `SELECT id,email,username,name,role,created_at,disabled_at FROM users WHERE id = ?`,
...
export function authenticate(identifier: string, password: string): User | null {
  ...
  return { id: u.id, email: u.email, username: u.username, name: u.name, role: u.role, created_at: u.created_at, disabled_at: u.disabled_at };
}
...
export async function currentUser(): Promise<User | null> {
  ...
  return { id: row.id, email: row.email, username: row.username, name: row.name, role: row.role, created_at: row.created_at, disabled_at: row.disabled_at };
}
...
export function listUsers(): User[] {
  return db().prepare(`SELECT id,email,username,name,role,created_at,disabled_at FROM users ORDER BY created_at`).all() as User[];
}
